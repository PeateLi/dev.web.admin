<template>
  <div class="w-table" :class="{'w-table_moving': dragState.dragging}">
    ...
    <!-- 当表头需要搜索的话  搜索不单独分开 -->
      ...
      <el-table-column v-if="option.isSearch">
        <!-- 自定义表头搜索栏 -->
        ...
        >
          <template slot-scope="scope">
            <div
              :class="{'cells':true,'active-row':scope.row.isChecked==true,'active-cell':scope.row.index === tabClickIndex && tabClickLabel === col.label&& col.isEdit !== false}"
            >
              <span
                v-if="scope.row.index === tabClickIndex && tabClickLabel === col.label && col.isEdit !== false"
              >
                <el-select
                ...
                <el-popover v-else-if="col.isSelectType == 'cron'" v-model="cronPopover">
                  <cron @change="changeCron" @close="inputBlur(scope.row,scope.row[col.prop])"></cron>
                  <el-input
                    slot="reference"
              ...
              <span v-else>
                <span v-if="scope.row[col.prop] == '' || scope.row[col.prop] == null"></span>
                <span v-else>
                 <span v-if="col.isTag  == 1">
                   <!-- {{scope.row[col.prop].split(";")}} -->
                   <el-tag v-for="item in scope.row[col.prop].split(';') " :key="item"> {{ item}}</el-tag>
                  </span> 
    ...
    <!-- 当表头不需要搜索的话  搜索单独发分开 -->
      <slot name="fixed"  v-if="option.isSearch == false"></slot>
      <el-table-column
      v-if="option.isSearch == false"
          v-for="(col, index) in tableHeader"
          :key="index"
          :prop="col.prop"
          :label="col.label"
...
export default {
  ...
  methods: {
    ...
    changeCron(row) {
      this.TableData[this.activeDataRow.index].cronExpressionString = row;
...
[data-v-348a3283]
 ...
 @media (max-width: 768px) {  
   /deep/ .is-group.has-gutter tr:first-child th{
