          ...
          <template slot-scope="scope">
            <div :class="scope.row.isChecked? 'active-row' :''">
              <span v-if="scope.row.index === tabClickIndex && tabClickLabel === '别名名称'">
                <el-input v-focus v-model="scope.row.mfgName" size="mini" @blur="inputBlur" />
              </span>
  ...
  methods: {
   getChangePage(val){   //接收分页子组件传过来的页数
    ...
    TableIsCheck(val) {
      ...
      for (let index = 0; index < _this.checkActiveDataRow.length; index++) {
            const e = _this.checkActiveDataRow[index];
            if(_this.array.length == 0){
              _this.array.push(e)
            }
        else{
        for (let index = 0; index < _this.array.length; index++) {
          const element = _this.array[index];
            if(e.mfgID == element.mfgID){
                 break
            }else{
              _this.array.push(e)
            }
      ...
      _this.array = _this.array.filter((x, index,self)=>self.indexOf(x)===index)  
    // console.log(val)
    ...
    EditData(row, column, cell, event){  //双击单元格修改数据
             switch (column.label) {
              case '别名名称':
     ...
     inputBlur(row, event, column) {// 失去焦点初始化
       let _this = this
      ...
      setMfgAlias(){
         ...
         let activeName =  _this.array.map(x => {
              return x.mfgName
          })
          let str = ''
          ...
          axios.post('/Utmel/Manufacturer/EditManufacturer',_this.AddNameRow)
          .then(res => {
  ...
  watch: {
    isDialog(val) {
      ...
      if (val == true) {
          ...
          _this.$nextTick(function(){    //通过监听 默认选中 表格别名
          if(this.AddNameRow.mfgAlias.indexOf('[') != -1){
            this.AddNameRow.mfgAlias = JSON.parse(this.AddNameRow.mfgAlias).join().replace(/,/g,";")
          }
               for (let index = 0; index < this.AddNameRow.mfgAlias.split(';').length; index++) {
